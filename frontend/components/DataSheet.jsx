...
function DataSheet({selectedBand,bandColors,setBandColors}){
    ...
    return(
        <table className={styles.datasheet}>
            ...
            <tbody>
                { isFourthBand ? <tr onClick={() => changeColor(null)}>
                    <td className={styles.noBand}>No Band</td>
                    <td>-</td>
                </tr> : null}
                {Object.entries(bands).map(([color,value],idx) => (
                <tr key={color} onClick={() => changeColor(color)}>
                    <td className={styles.colorCell}><div className={styles.colorBox} style={{"backgroundColor":color}}></div>{color}</td>
                    <td>{value}{isToleranceBand && "%"}</td>
                </tr>
                ))}
            </tbody>
            </table>
